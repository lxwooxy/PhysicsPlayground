...
public class Player : MonoBehaviour
{
    //Variables
    [Header("References")]
    [Tooltip("Reference to the root Transform, on whick the player script is attached.")]
    ...
    //Y velocity is sotred in a separate float, apart from the velocity vector:
    ...
    [Tooltip("Time taken, in seconds, to go from moving at full speed to a standstill.")]
    public float timeToLoseMaxSpeed = .2f;

    [Tooltip("Multiplier for additional velocity gain when moving against ongoing momentum. For example, 0 means no additional velocity, .5 means 50% extra.")]
    public float reverseMomentumMultiplier = .6f;

    [Tooltip("Multiplier for velocity influence when moving while in midair. For example, .5 means 50% speed. A value greater than 1 will make you move faster while midair.")]
    public float midairMovementMultiplier = .4f;

    [Tooltip("Multiplier for how much velocity is retained after bouncing off a wall. For example, 1 is full velocity, 2 is 20%.")]
    [Range(0,1)]
    ...
    [Tooltip("Maximum distance from the players side that a wall can be detected for a wall jump.")]
    ...
    void Movement()
    {
        ...
        if (localMovementDirection != Vector3.zero)
        {
            ...
            //0 if moving perpenticular to existing velocity, 
            ...
            //Exactly 1 means moving directly away from the existing momentum. 
            //Thus, we'll get the full reversMomentumMultiplier only when it's 1.
            multiplier += -dot * reverseMomentumMultiplier;

            //Calculate the new velocity by adding movement velocity to the current celocity:
    ...
    void VelocityLoss()
    {
        //Lose velocity as long as we're grounded, and we either are not holding movement keys, or are moving faster than 'movespped.:
        if (grounded && (localMovementDirection == Vector3.zero || worldVelocity.magnitude > movespeed))
        {
            ...
            //If we're losing more velocity than the world velocity magnitue:
    ...
    void Gravity()
    {
        ...
        if ( !grounded && yVelocity > -maxGravity)
            //Decrease Y velocity by GravityPerSecond, but don't go below max Gravity
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                if (WallIsNearby())
                {
                    ...
                    if (localMovementDirection != Vector3.zero)
                        ...
                        //Have ongoing momentum be retained when a wall jump occurs:
                        //worldVelocity...(localMovementDirection) * (wallJumpPower + worldVelocity.magnitude);
    ...
    void Jumping()
    {
        ...
        {
            //Start travelling 'jumpPower' upwards per second:
            yVelocity = jumpPower;
    ...
    void ApplyVelocity()
    {
        ...
        //Caluclate the movement we'll receive this frame:
        ...
        //Bounce off the walls when we hit our sides while midair:
