...
public class PlatformMovement : MonoBehaviour
{
    ...
    [Tooltip("TheRigidbody of the platform.")]
    public Rigidbody rb;
    ...
    [Tooltip("The amount of time taken to move from one position to the other.")]
    ...
    //Gets the current position we're moving towards based on the state:
    ...
    void FixedUpdate()
    {
        if (state != State.Stationary)
        {
            ...
            if (distanceMovedThisFrame >= DistanceToDestination)
            {
                ...
                //Based on our current state, determine what our next state will be:
                if (state == State.MovingToInitial)
