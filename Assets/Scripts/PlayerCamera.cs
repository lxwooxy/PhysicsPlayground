...
public class PlayerCamera : MonoBehaviour
{
    ...
    [Tooltip("Set this to the Transform that has the Camera component(which should also have the PlayerCameracomponent).")]
    ...
    [Tooltip("the amount of smoothing applied to the first-person camera. A highervalue will cause the camera to more gradually turn when the mouse is moved.")]
    [Range(0,.99f)]
    ...
    //Bounds
    [Header("Bounds")]
    [Tooltip("Minimum distance for the third person camera to have from its orbit point.")]
    public float minThirdPersonDistance = 5;
    
    [Tooltip("Maximum distance for the third person camera to have from its orbit point.")]
    ...
    [Tooltip("X euler angles for the camera when its looking as far down as it can.")]
    ...
    [Header("Misc")]
    [Tooltip("The layer mask for what the third-person camera will be obstructed by, and what it will ignore and pass through. You'll probably want this to include enviromental objects, but not smaller entities.")]
    ...
    //Target position for t he third-personcamera.
    ...
    void Hotkeys()
    {
        ...
        //Scroll whell for the third- person distance:
        if (!firstPerson) //Only check for it while we're in third-person mode
        {
            //Get scroll whell delta this frame:
    ...
    {
        //The X rotation we should be recieving uses the mouse Y because rotationg along the X axis makes us look up/down.
        float xRotation = (Input.GetAxis("Mouse Y") * -rotationSpeed);

        //The Y rotation we should be recieving uses the mouse X because the rotation along the Y axis makes us look left/right.
        ...
        else //If the X rotation is anywhere between the 0 and 180
        {
    ...
    void FirstPerson()
    {
        //If the camera isnt in its first-person location, move it there:
        ...
        //Apply just the Z and Y axes to the camera:
        trans.eulerAngles = new Vector3(targetRotation.eulerAngles.x,targetRotation.eulerAngles.y,0);

        //Make the model face the same direction as the camera, but with the Y axis removed from the direction:
    ...
    void ThirdPerson()
    {
        ...
        if (Physics.Raycast(ray,out hit, thirdPersonDistance + wallMargin, thirdPersonRayLayermask.value))
        {
            ...
            //We'll offset it back towards the cameraTarget bu 'wall Margin' distance:
