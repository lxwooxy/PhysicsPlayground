...
public class Telekinesis : MonoBehaviour
{
    ...
    //Regidbody component of targer. For something to be marked as a target, it must have a Rigidbody.
    //So as long as "target" is not null, this wont be null either
    private Rigidbody targetRigidbody;

    //If there is no current targer, this is always false. Otherwise, true if the target is in range, false if they are not.
    ...
    {
        //Get a ray going out of the center of the screem:
        ...
        if (Physics.Raycast(ray,out hit, Mathf.Infinity,detectionLayerMask.value))
        {
            ...
            if (hit.rigidbody != null && !hit.rigidbody.isKinematic) // and it has a non- kinematic Rigidbody,
            {
                ...
                //Based on distance, set targetIsOutsitRange:
    ...
    //FixedUpdate logic: use when interacting with physics system through code/applying forces to Rigidbodies.
    //It occurs not once per frame, but at a set interval with the same amount of time between each FixedUpdate.
    //Edit > Project Settings > Time tab > Fixed Timestep value. (.02 = 50 times per second)
    void PullingAndPushing()
    {
        ...
        if (target != null && !targetIsOutsideRange)
        {
            ...
            {
                //Pull the target from the hit point towards out position:
    ...
    void OnGUI()
    {
        ...
        //Can't build a real game with EditorGUI. 
        //Implement cursor with an actual UI element like a panel, changing colors through references.
    }
}
